...
subword_output_iterator::subword_output_iterator(unsigned width)
	...
	_backing = std::make_shared<std::vector<uint8_t>>();
	if (0 != 8 % width) {
		throw new std::invalid_argument("subword_output_iterator only supports power-of-two widths less than 8");
	}
}
...
std::vector<uint8_t> subword_output_iterator::result() {
...
void subword_output_iterator_deref::operator=(unsigned new_value) {
	...
	uint8_t mask = (1 << _width) - 1;

	uint8_t buffer = (*(this->_backing))[_word];
