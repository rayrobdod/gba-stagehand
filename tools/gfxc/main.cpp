...
int main(int argc, char* argv[]) {
	...
	std::vector<std::vector<rgba16_t>> sprite_palettes;
	{
		std::set<std::set<rgba16_t>> sprite_palettes_0;
		for (auto const& image : sprite_imgs) {
			...
			sprite_palettes_0.insert(new_pal);
		}

		// TODO: more deduplication

		for (auto pal0 : sprite_palettes_0) {
			std::vector pal(pal0.begin(), pal0.end());
			sprite_palettes.push_back(pal);
		}
	}
	...
	for (auto const& image : sprite_imgs) {
		...
		uint16_t paltag;
		for (paltag = 0; paltag < sprite_palettes.size(); ++paltag) {
			const std::vector<rgba16_t> check_pal = sprite_palettes[paltag];

			if (std::includes(check_pal.begin(), check_pal.end(), innate_pal.begin(), innate_pal.end())) {
				break;
			}
		}
		if (paltag >= sprite_palettes.size()) {
			std::string msg("Lost this sprite's palette");
			throw std::logic_error(msg);
		}
		const std::vector<rgba16_t> used_pal = sprite_palettes[paltag];
		paltag += FIRST_TAG;
	...
	for (auto pal0 = sprite_palettes.begin(); pal0 != sprite_palettes.end(); ++pal0) {
		size_t i = pal0 - sprite_palettes.begin();
		char pal_name[16];
